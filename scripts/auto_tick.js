...
async function run() {
  console.log('Fetching documents from today...');
  ...
  if (!docs || docs.length === 0) {
    console.log('No documents found today.');
    return;
  }
  
  console.log(`Found ${docs.length} documents. Mapping to checklists...`);
  
  const clientTicks = {}; // clientId -> Set of checklist IDs
  ...
  console.log('Mapped updates for clients:', Object.keys(clientTicks).length);
  ...
  for (const clientId of Object.keys(clientTicks)) {
    ...
    if (clientData) {
      ...
      if (merged.length > currentList.length) {
        console.log(`Updating client ${clientId} - added ${merged.length - currentList.length} ticks.`);
        await supabase
          .from('clients')
  ...
  console.log('Finished restoring ticks!');
}
