...
async function run() {
  console.log("Fetching ALL historical documents to fully restore stages and tasks...");
    ...
    if (phases && phases.length > 0) {
      ...
      for (const p of phases) {
        ...
        if (typeof p.tasks === 'string') {
          try { newTasks = JSON.parse(p.tasks); } catch(e) {}
        } else {
          newTasks = p.tasks || [];
        }
        ...
        if (pModified) {
          console.log(`Restoring ticks for phase ${p.name} for client ${clientId}`);
          await supabase
            .from('phases')
            .update({ 
  ...
  console.log(`Done! Fully restored ${tasksUpdatedCount} tasks across ${phasesUpdatedCount} phases by analyzing all historical files.`);
}
run();
