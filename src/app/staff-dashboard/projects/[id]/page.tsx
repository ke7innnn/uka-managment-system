    ...
    try {
      await Promise.all(files.map(file => new Promise<void>(resolve => {
        const upload = async (blob: Blob, ext: string, mime: string) => {
          ...
          if (error) { alert('Upload failed: ' + error.message); resolve(); return; }
          const { data: { publicUrl } } = supabase.storage.from('uka-storage').getPublicUrl(path);
          const doc: Doc = { id: crypto.randomUUID(), name: file.name, url: publicUrl, uploadedAt: new Date().toISOString(), type: mime, size: blob.size, folder: folderId, uploadedBy: currentStaffId || undefined };
          const c = getClientById(params.id);
              ...
              {(client.kyc.northPhoto || client.kyc.northDetails || client.kyc.southPhoto || client.kyc.southDetails || client.kyc.eastPhoto || client.kyc.eastDetails || client.kyc.westPhoto || client.kyc.westDetails || client.kyc.road || client.kyc.roadDetails || client.kyc.side || client.kyc.sideDetails || client.kyc.digitalSignaturePhoto || client.kyc.clientAadharPhoto || client.kyc.clientPanPhoto) && (
                <div style={{ marginTop: '1.5rem', borderTop: '1px solid var(--border)', paddingTop: '1.5rem' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '1rem' }}>
                    {client.kyc.clientAadharPhoto && (<FilePreviewCard src={client.kyc.clientAadharPhoto} label="Client Aadhaar Photo" />)}
                    {client.kyc.clientPanPhoto && (<FilePreviewCard src={client.kyc.clientPanPhoto} label="Client PAN Photo" />)}
                    {client.kyc.northPhoto && (<FilePreviewCard src={client.kyc.northPhoto} label="North Side Photo" />)}
                    {client.kyc.northDetails && (<FilePreviewCard src={client.kyc.northDetails} label="North Direction Details" />)}
                    {client.kyc.southPhoto && (<FilePreviewCard src={client.kyc.southPhoto} label="South Side Photo" />)}
                    {client.kyc.southDetails && (<FilePreviewCard src={client.kyc.southDetails} label="South Direction Details" />)}
                    {client.kyc.eastPhoto && (<FilePreviewCard src={client.kyc.eastPhoto} label="East Side Photo" />)}
                    {client.kyc.eastDetails && (<FilePreviewCard src={client.kyc.eastDetails} label="East Direction Details" />)}
                    {client.kyc.westPhoto && (<FilePreviewCard src={client.kyc.westPhoto} label="West Side Photo" />)}
                    {client.kyc.westDetails && (<FilePreviewCard src={client.kyc.westDetails} label="West Direction Details" />)}
                    {client.kyc.road && (<FilePreviewCard src={client.kyc.road} label="Road Photo" />)}
                    {client.kyc.roadDetails && (<FilePreviewCard src={client.kyc.roadDetails} label="Road Details" />)}
                    {client.kyc.side && (<FilePreviewCard src={client.kyc.side} label="Side Photo" />)}
                    {client.kyc.sideDetails && (<FilePreviewCard src={client.kyc.sideDetails} label="Side Details" />)}
                    {client.kyc.digitalSignaturePhoto && (<FilePreviewCard src={client.kyc.digitalSignaturePhoto} label="Digital Signature Photo" />)}
                  </div>
                </div>
              )}
                      ...
                      <div key={owner.id || idx} style={{ border: '1px solid var(--border)', borderRadius: '8px', padding: '1rem', background: 'rgba(255, 255, 255, 0.01)' }}>
                        ...
                        {(owner.aadharPhoto || owner.panPhoto) && (
                          <div style={{ display: 'flex', gap: '1rem', marginTop: '0.75rem' }}>
                            {owner.aadharPhoto && (<FilePreviewCard src={owner.aadharPhoto} label="Aadhaar Photo" height={70} />)}
                            {owner.panPhoto && (<FilePreviewCard src={owner.panPhoto} label="PAN Photo" height={70} />)}
                          </div>
                        )}
                      </div>
                    ))}
          ...
          <div className={`glass-panel ${styles.progressCard}`} style={{ marginBottom: '2rem' }}>
            <div className={styles.progressFlex}>
              <div>
                ...
                <p style={{ fontSize: '0.825rem', color: 'var(--text-muted)', marginTop: '0.25rem' }}>
                  View and manage CC, RDP, OC, and PCC certificates and documents.
                </p>
              </div>
            </div>
          </div>
...
function FilePreviewCard({ src, label, height = 110 }: { src: string; label: string; height?: number }) {
  ...
  // ── Detect real MIME from data URL prefix (most reliable) ────────────────
  ...
  const handleView = () => {
    ...
    if (src.startsWith('data:')) {
      const viewMime = isHeic || isOctet ? 'image/jpeg' : mime;
