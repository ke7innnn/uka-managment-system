    ...
    <div style={{ fontFamily: "'Times New Roman', Georgia, serif", maxWidth: 900, margin: "0 auto", padding: "1rem", color: C.ink, background: "#fff", borderRadius: 8 }}>
      <style>{`
        @media print {
          ...
          .bill-outer { border-color: ${C.ink} !important; }
        }
      ...
      <div className="bill-outer" style={{ border: `2px solid ${C.ink}` }}>
        ...
        {/* Header */}
        <div style={{ background: C.ink, color: "#fff", textAlign: "center", padding: "16px 20px" }}>
        ...
        {/* GSTIN / PAN */}
        <div style={{ display: "flex", justifyContent: "space-between", background: C.stripe2, padding: "5px 16px", borderBottom: `1px solid ${C.border}`, flexWrap: "wrap", gap: 4 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", borderBottom: `1px solid ${C.border}` }}>
          <div style={{ padding: "10px 16px", borderRight: `1px solid ${C.border}` }}>
            ...
            <input style={inp({ fontSize: 12, fontWeight: 600 })} value={form.billTo} onChange={e => updateField("billTo", e.target.value)} placeholder="Client Name" />
          </div>
          <div style={{ padding: "10px 16px" }}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
              <div>
                <div style={{ fontSize: 10, color: C.muted, marginBottom: 2 }}>Invoice No.</div>
                <input style={inp()} value={form.invoiceNo} onChange={e => updateField("invoiceNo", e.target.value)} />
              </div>
              <div>
                <div style={{ fontSize: 10, color: C.muted, marginBottom: 2 }}>Date</div>
                <input type="date" style={inp()} value={form.date} onChange={e => updateField("date", e.target.value)} />
              </div>
              <div style={{ gridColumn: "1/-1" }}>
                <div style={{ fontSize: 10, color: C.muted, marginBottom: 2 }}>Due Date (optional)</div>
                <input type="date" style={inp()} value={form.dueDate} onChange={e => updateField("dueDate", e.target.value)} />
              </div>
            </div>
          </div>
        </div>
        ...
        {/* Subject */}
        <div style={{ padding: "8px 16px", borderBottom: `1px solid ${C.border}`, background: C.soft }}>
          <div style={{ marginBottom: 6 }}>
            <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.04em" }}>SUB: </span>
            <textarea style={{ ...inp(), resize: "vertical", minHeight: 34, fontFamily: "inherit", fontSize: 11 }} value={form.subject} onChange={e => updateField("subject", e.target.value)} rows={2} />
          </div>
        ...
        <div style={{ padding: "12px 16px" }}>
            ...
            return (
              <div key={bIdx} style={{ marginBottom: 20 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
                  <input style={{ ...inp({ width: 200, fontWeight: 700, fontSize: 12, letterSpacing: "0.03em" }) }} value={bldg.name} onChange={e => updateBuildingName(bIdx, e.target.value)} />
                  {buildings.length > 1 && (
                ...
                <table style={{ width: "100%", borderCollapse: "collapse" }}>
                  <thead>
                    <tr>
                      <th style={TH({ width: 32 })}>Sr.</th>
                      <th style={TH({ textAlign: "left" })}>Floor</th>
                      <th style={TH()}>Area (Sq. Mtr)</th>
                      <th style={TH()}>No. of Floors</th>
                      <th style={TH()}>Total (Sq. Mtr)</th>
                      <th style={TH()}>Total (Sq. Ft)</th>
                      <th style={TH({ width: 32 })} className="np"></th>
                    </tr>
                  </thead>
                  <tbody>
                    {bt.rows.map((row, fIdx) => (
                      <tr key={fIdx} className="erow">
                        ...
                        <td style={TD("left")}>
                          <input style={inp()} value={bldg.floors[fIdx].floor} onChange={e => updateFloor(bIdx, fIdx, "floor", e.target.value)} placeholder="Floor name" />
                        </td>
                        <td style={TD()}>
                          <input type="number" step="0.001" style={inp({ textAlign: "right" })} value={bldg.floors[fIdx].areaSqmt} onChange={e => updateFloor(bIdx, fIdx, "areaSqmt", e.target.value)} />
                        </td>
                        <td style={TD()}>
                          <input type="number" min="1" style={inp({ textAlign: "center" })} value={bldg.floors[fIdx].noOfFloors} onChange={e => updateFloor(bIdx, fIdx, "noOfFloors", e.target.value)} />
                        </td>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", marginTop: 2 }}>
            <tbody>
              <tr>
                <td colSpan={4} style={{ ...TD("right", C.ink), color: "#fff", fontWeight: 700, border: `1px solid ${C.mid}`, letterSpacing: "0.04em", fontSize: 11 }}>TOTAL BLDG-1 & BLDG-2</td>
                <td style={{ ...TD("right", C.ink), color: "#fff", fontWeight: 700, border: `1px solid ${C.mid}` }}>{fmt(grandSqmt)}</td>
                <td style={{ ...TD("right", C.ink), color: "#fff", fontWeight: 700, border: `1px solid ${C.mid}` }}>{fmt(grandSqft)}</td>
                <td className="np" style={{ ...TD(), background: C.ink, border: `1px solid ${C.mid}` }}></td>
              </tr>
            </tbody>
          </table>
        </div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr>
                ...
                <th style={TH({ width: 120 })}>Formula</th>
                <th style={TH({ width: 175 })}>Amount Details</th>
                <th style={TH({ width: 40 })} className="np"></th>
              </tr>
            </thead>
            <tbody>
              {[
                { lbl: "A.", desc: "Percentage %", formula: "", cell: <input type="number" step="0.01" style={inp({ textAlign: "right" })} value={form.percentageA} onChange={e => updateField("percentageA", e.target.value)} />, bg: C.paper },
                { lbl: "B.", desc: "Construction Area In Sq. ft", formula: "Auto", cell: <span style={{ fontWeight: 600 }}>{fmt(grandSqft)} SQ.FT</span>, bg: C.stripe },
                { lbl: "C.", desc: "Liasoning Fees Per Sq. ft", formula: "", cell: <input type="number" style={inp({ textAlign: "right" })} value={form.liasoningFeePerSqft} onChange={e => updateField("liasoningFeePerSqft", e.target.value)} />, bg: C.paper },
                ...
                <tr key={lbl}>
                  ...
                  <td className="np" style={TD("center", bg)}></td>
                </tr>
              ))}

              <tr>
                <td style={{ ...TD(), fontWeight: 700 }}>G.</td>
                <td style={TD("left")}>GST <input type="number" style={inp({ display: "inline", width: 46 })} value={form.gstRate} onChange={e => updateField("gstRate", e.target.value)} />% of (D)</td>
              ...
              <tr>
                ...
                <td style={TD("left")}>TDS <input type="number" style={inp({ display: "inline", width: 46 })} value={form.tdsRate} onChange={e => updateField("tdsRate", e.target.value)} />% of (D)</td>
              ...
              <tr>
                ...
                <td style={TD("right")}><input type="number" style={inp({ textAlign: "right" })} value={form.prevPaid} onChange={e => updateField("prevPaid", e.target.value)} /></td>
                <td className="np" style={TD()}></td>
              </tr>
              ...
              <tr>
                ...
                <td className="np" style={{ ...TD(), background: C.stripe2, borderTop: `2px solid ${C.ink}` }}></td>
              </tr>
              <tr>
                ...
                <td className="np" style={TD("center", C.stripe2)}></td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        {/* Amount in Words */}
        <div style={{ padding: "8px 16px", background: C.stripe, borderTop: `1px solid ${C.border}` }}>
